...
export default function RegisterPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (!reg.ok) {
      ...
      setError(
        data.detail === "Email déjà enregistré"
  ...
  return (
    <main className="min-h-screen flex bg-[#fafaf9]">
      ...
      {/* ── Panneau gauche décoratif premium ── */}
      <div className="hidden lg:flex flex-col justify-between w-[420px] shrink-0 bg-[#1a1a1a] p-10 relative overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-[#D32F2F]/15 border border-[#D32F2F]/30 text-[#ff8a75] text-[12px] font-semibold mb-6 leading-none">
            <span className="w-1.5 h-1.5 rounded-full bg-[#D32F2F] animate-pulse shrink-0" />
            Inscription Marchand
          </div>
          <h2 className="text-white font-extrabold text-[28px] leading-snug tracking-tight mb-3">
            Ouvrez votre boutique<br />en 2 minutes chrono
          </h2>
          <p className="text-white/50 text-[14px] leading-relaxed">
            Rejoignez des dizaines de restaurateurs et commerçants qui vendent chaque jour en direct sur WhatsApp.
          </p>
        </div>
        ...
        <div className="relative z-10 flex flex-col gap-2.5">
          {[
            ...
            "📱 Catalogue optimisé mobile & tablette",
            "💵 Encaissement direct à la livraison",
            "🛡️ Isolation sécurisée de vos données"
          ].map((f) => (
        ...
        <div className="w-full max-w-[400px]">
          ...
          <div className="mb-8 text-center lg:text-left">
            ...
            <p className="text-[#888] text-[14px]">
              Démarrez la gestion de vos établissements
            </p>
          </div>
          ...
          <form onSubmit={submit} className="flex flex-col gap-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="h-12 rounded-xl bg-[#D32F2F] text-white font-bold text-[14px] shadow-[0_4px_14px_rgba(211,47,47,0.4)] hover:bg-[#B71C1C] active:scale-95 transition-all disabled:opacity-60 mt-1 leading-none flex items-center justify-center"
            >
            ...
            <p className="text-[13px] text-center text-[#888] mt-1">
              Déjà inscrit ?{" "}
              <Link href="/admin/login" className="text-[#D32F2F] font-semibold hover:underline">
                Se connecter
              </Link>
            </p>
          </form>
          ...
          <div className="mt-8 pt-6 border-t border-[#eae7e7] text-center">
            <Link href="/kubafoodies" className="inline-flex items-center gap-1.5 text-[#888] text-[13px] hover:text-[#1a1a1a] transition-colors leading-none">
              <span>←</span>
